...
TEST(crosswordBuilderTest, impossibleToBuildTest)
{
    ...
    {
        toUpper(inp.begin(), inp.end());
        testUtils::testException([&inp]() { CrosswordBuilder::build(inp, inp.size()); }, "Impossible to build a crossword which consists of 6 words from this.");
    };

    test(std::vector<std::string>{"veldt", "jynx", "grimps", "waqf", "zho", "buck"});
    testUtils::testException([]() { CrosswordBuilder::build(toUpper({ "sophistication", "cat" }), 2, 13); }, "The input consists of too long words.");
}
